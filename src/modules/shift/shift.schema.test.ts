...
describe('Shift Schemas', () => {
    ...
    describe('startShiftSchema', () => {
        it('should validate valid payload', () => {
            const result = startShiftSchema.safeParse({ start_cash: 50000 });
            expect(result.success).toBe(true);
        });

        it('should reject negative start_cash', () => {
            const result = startShiftSchema.safeParse({ start_cash: -1 });
        ...
        it('should reject start_cash > 100 million', () => {
            const result = startShiftSchema.safeParse({ start_cash: 100000001 });
            expect(result.success).toBe(false);
            if (!result.success) {
    ...
    describe('endShiftSchema', () => {
        ...
        it('should validate payload without optional notes', () => {
            const result = endShiftSchema.safeParse({ end_cash: 150000 });
            expect(result.success).toBe(true);
        });

        it('should reject negative end_cash', () => {
            const result = endShiftSchema.safeParse({ end_cash: -1 });
            expect(result.success).toBe(false);
        });
    ...
    describe('shiftIdParamSchema', () => {
        it('should validate valid UUID', () => {
            const result = shiftIdParamSchema.safeParse({
        ...
        it('should reject invalid UUID', () => {
            const result = shiftIdParamSchema.safeParse({ shift_id: 'invalid-id' });
            expect(result.success).toBe(false);
        });
    });
    ...
    describe('shiftListQuerySchema', () => {
        it('should validate valid query params', () => {
            const result = shiftListQuerySchema.safeParse({
                batch: '1',
                size: '10',
                is_active: 'true',
        ...
        it('should reject invalid date format', () => {
            const result = shiftListQuerySchema.safeParse({ start_date: '01-01-2024' });
            expect(result.success).toBe(false);
        });

        it('should reject invalid user_id UUID', () => {
            const result = shiftListQuerySchema.safeParse({ user_id: 'invalid' });
            expect(result.success).toBe(false);
        });
    });
});
