...
import { mockCreateUserData } from '../../tests/mocks/user.mock';
...
describe('User Schema Validation', () => {
    describe('createUserSchema', () => {
        it('should pass validation with valid required fields', () => {
            const result = createUserSchema.safeParse(mockCreateUserData.valid);
        ...
        it('should pass validation with optional user_status_id', () => {
            const result = createUserSchema.safeParse(mockCreateUserData.withOptionalStatus);
        ...
        it('should fail validation with invalid UUID format for role_id', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const roleIdError = result.error.issues.find(
                    (issue) => issue.path.includes('role_id')
                );
        ...
        it('should fail validation with invalid UUID format for user_status_id', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const statusIdError = result.error.issues.find(
                    (issue) => issue.path.includes('user_status_id')
                );
        ...
        it('should fail validation with invalid UUID format', () => {
            ...
            if (!result.success) {
                const userIdError = result.error.issues.find(
                    (issue) => issue.path.includes('user_id')
                );
    ...
    describe('Enums', () => {
        describe('RoleName', () => {
            ...
            it('should have CASHIER value', () => {
                expect(RoleName.CASHIER).toBe('CASHIER');
            });
        });

        describe('UserStatusName', () => {
            it('should have ACTIVE value', () => {
                expect(UserStatusName.ACTIVE).toBe('ACTIVE');
            });

            it('should have INACTIVE value', () => {
                expect(UserStatusName.INACTIVE).toBe('INACTIVE');
            });

            it('should have DELETED value', () => {
                expect(UserStatusName.DELETED).toBe('DELETED');
            });
        });
    });
});
