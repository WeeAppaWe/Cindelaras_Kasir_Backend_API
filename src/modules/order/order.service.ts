...
export const getAll = async (req: AuthenticatedRequest): Promise<OrderListResponse> => {
    try {
        ...
        const [data, totalData] = await Promise.all([
            orderRepository.findAll(options, filter),
        ...
        return {
            ...
            records: data as OrderWithDetails[],
        };
    } catch (error) {
        console.error(`--- Order Service Error: ${(error as Error).message}`);
        throw error;
    }
};
...
export const getDetail = async (req: AuthenticatedRequest): Promise<OrderWithDetails> => {
    try {
        ...
        if (!order) {
            throw new ErrorNotFoundException('Pesanan tidak ditemukan');
        }
 ...
 */
export const create = async (req: AuthenticatedRequest): Promise<CreateOrderResponse> => {
    try {
        ...
        const activeShift = await orderRepository.getActiveShift(metadata.account_id);
        if (!activeShift) {
        ...
        for (const item of body.items) {
            const menu = await orderRepository.findMenuById(item.menu_id);

            if (!menu) {
                throw new ErrorValidationException(`Menu dengan ID ${item.menu_id} tidak ditemukan`, [
                    { location: 'body', field: 'items', message: `Menu tidak ditemukan` },
                ]);
            }

            if (!menu.is_available) {
                throw new ErrorValidationException(`Menu "${menu.name}" tidak tersedia`, [
        ...
        const fullOrder = await orderRepository.findByIdWithDetails(result.order_id);
 ...
 */
export const confirmPayment = async (req: AuthenticatedRequest): Promise<ConfirmPaymentResponse> => {
    try {
        const orderId = req.params.order_id;
        const body = req.body;
        ...
        if (order.status !== OrderStatus.PENDING) {
            throw new ErrorValidationException('Pesanan tidak dalam status menunggu pembayaran', [
                {
                    location: 'params',
                    field: 'order_id',
        ...
        if (order.payment_type === PaymentType.CASH) {
            ...
            if (!paidAmount || paidAmount <= 0) {
                throw new ErrorValidationException('Jumlah bayar wajib diisi untuk pembayaran tunai', [
                    { location: 'body', field: 'paid_amount', message: 'Jumlah bayar wajib diisi' },
                ]);
            }

            if (paidAmount < order.total_amount) {
                throw new ErrorValidationException('Jumlah bayar kurang dari total', [
                    {
                        location: 'body',
                        field: 'paid_amount',
        ...
        for (const orderItem of order.order_items) {
            ...
            const menu = await orderRepository.findMenuById(orderItem.menu_id);
            if (menu && menu.recipes) {
                for (const recipe of menu.recipes) {
                    ...
                    stockDeductions.set(
                        ...
                        currentDeduction + (recipe.qty_needed * orderItem.qty)
                    );
                }
            }
        }
        ...
        const receiptNumber = order.receipt || generateReceiptNumber(order.order_id, order.created_at);
        ...
        await getPrismaClient().$transaction(async (transaction) => {
            ...
            await orderRepository.confirmOrder(orderId, paidAmount, changeAmount, receiptNumber, transaction);

            // 2. Deduct ingredient stocks and create stock movement records
            for (const [ingredientId, deductAmount] of stockDeductions) {
                await orderRepository.updateIngredientStock(
                    ingredientId,
                    deductAmount,
                    order.user_id,
        ...
        // Build message
        const message = order.payment_type === PaymentType.CASH
            ? `Pembayaran berhasil. Kembalian: ${formatMoney(changeAmount)}`
 ...
 */
export const cancelOrder = async (req: AuthenticatedRequest): Promise<CancelOrderResponse> => {
    try {
        const orderId = req.params.order_id;
        ...
        if (order.status !== OrderStatus.PENDING) {
            throw new ErrorValidationException('Hanya pesanan dengan status PENDING yang dapat dibatalkan', [
                {
                    location: 'params',
                    field: 'order_id',
                    message: `Status pesanan: ${order.status}`,
...
const getReceiptStoreInfo = async (): Promise<ReceiptStoreInfo> => {
 ...
 */
export const getReceipt = async (req: AuthenticatedRequest) => {
    try {
        const orderId = req.params.order_id;
        ...
        switch (format) {
            case 'escpos':
                ...
                break;
            case 'pdf':
                content = await receiptUtility.generatePdfReceipt(receiptData);
                break;
            case 'image':
                content = receiptUtility.generateHtmlReceipt(receiptData);
                break;
            default:
...
const generateTextReceipt = (data: {
    ...
    data.items.forEach((item) => {
        ...
        const itemLine = `  ${item.qty} x ${formatMoney(item.price)}`;
    ...
    addTotalLine('TOTAL', formatMoney(data.total));
    addTotalLine(`Bayar (${data.payment_type})`, formatMoney(data.paid_amount));
    if (data.change_amount > 0) {
        addTotalLine('Kembalian', formatMoney(data.change_amount));
    }
    lines.push(line);
...
const generateEscPosReceipt = (data: {
    ...
    data.items.forEach((item) => {
        ...
        receipt += `  ${item.qty} x ${formatMoney(item.price)}`;
        receipt += `  ${formatMoney(item.subtotal)}` + LF;
    });
    receipt += '--------------------------------' + LF;

    // Totals
    receipt += BOLD_ON;
    receipt += `TOTAL: ${formatMoney(data.total)}` + LF;
    receipt += BOLD_OFF;
    receipt += `Bayar (${data.payment_type}): ${formatMoney(data.paid_amount)}` + LF;
    if (data.change_amount > 0) {
        receipt += `Kembalian: ${formatMoney(data.change_amount)}` + LF;
    }
    receipt += '================================' + LF;
