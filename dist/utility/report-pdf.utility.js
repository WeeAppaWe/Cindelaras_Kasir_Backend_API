...
const createReportFileResult = (data, options, buffer) => {
    return {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const drawMetadata = (metrics = []) => {
                const metadata = [];
                if (data.period) {
                    metadata.push({ label: 'Periode Mulai', value: data.period.start_date, format: 'date' }, { label: 'Periode Selesai', value: data.period.end_date, format: 'date' });
                }
            ...
            const drawTableHeader = (table, columnWidths) => {
            ...
            const drawSection = (sectionTitle) => {
                ensureSpace(34);
                doc
                    .font('Helvetica-Bold')
                    .fontSize(13)
            ...
            for (let index = 0; index < totalPages; index++) {
                ...
                doc
                    ...
                    .fontSize(8)
                    .fillColor('#6B7280')
                    .text(`Halaman ${index + 1} dari ${totalPages}`, PAGE_MARGIN, pageHeight - PAGE_MARGIN + 8, { width: contentWidth, align: 'right' });
            }
            doc.end();
        }
        catch (error) {
...
//# sourceMappingURL=report-pdf.utility.js.map