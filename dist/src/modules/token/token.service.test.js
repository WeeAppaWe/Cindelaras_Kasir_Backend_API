...
describe('Token Service', () => {
    ...
    describe('generateAccess', () => {
        it('should generate an access key from user ID', async () => {
            ...
            const result = await token_service_1.default.generateAccess(userId);
            expect(result).toBeDefined();
            expect(typeof result).toBe('string');
            expect(result.length).toBe(32); // MD5 hash length
        });
        it('should generate different keys for different inputs', async () => {
            const result1 = await token_service_1.default.generateAccess('user1');
            const result2 = await token_service_1.default.generateAccess('user2');
    ...
    describe('generatePublicToken', () => {
        it('should generate public key and token', () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.encrypt.mockReturnValue('encrypted-token');
            // Act
            const result = token_service_1.default.generatePublicToken('test-access-key');
            // Assert
    ...
    describe('storeTokenToRedis', () => {
        it('should store token in Redis with TTL', async () => {
            ...
            // Act
            const result = await token_service_1.default.storeTokenToRedis('test-key', mockRedisTokenPayload);
            // Assert
            expect(result).toBe(true);
            expect(redis_connection_1.default.set).toHaveBeenCalledWith('test-key', JSON.stringify(mockRedisTokenPayload), 'EX', expect.any(Number));
        });
        it('should throw error when Redis fails', async () => {
            // Arrange
            redis_connection_1.default.set.mockRejectedValue(new Error('Redis connection failed'));
            // Act & Assert
            await expect(token_service_1.default.storeTokenToRedis('test-key', mockRedisTokenPayload))
                .rejects
                .toThrow();
        });
    });
    describe('getAccessKey', () => {
        it('should decode token and return access key with public key', () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue('access-key-123-public-key-456');
            // Act
            const result = token_service_1.default.getAccessKey('encrypted-token');
            // Assert
        ...
        it('should throw error for invalid token format', () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue(null);
            // Act & Assert
            expect(() => token_service_1.default.getAccessKey('invalid-token'))
    ...
    describe('checkAuthToken', () => {
        it('should return user ID for valid token', async () => {
            ...
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue('accesskey123-pubkey456');
            redis_connection_1.default.get.mockResolvedValue(JSON.stringify({
            ...
            const result = await token_service_1.default.checkAuthToken('encrypted-token', 'pubkey456');
            // Assert
            expect(result).toBe(mockRedisTokenPayload.id);
        });
        it('should throw error when keys do not match', async () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue('accesskey-wrongkey');
            // Act & Assert
            await expect(token_service_1.default.checkAuthToken('encrypted-token', 'different-key'))
                .rejects
                .toThrow(error_authentication_exception_1.ErrorAuthenticationException);
        });
        it('should throw error when token not found in Redis', async () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue('accesskey-mypubkey');
            redis_connection_1.default.get.mockResolvedValue(null);
            // Act & Assert
            await expect(token_service_1.default.checkAuthToken('encrypted-token', 'mypubkey'))
                .rejects
                .toThrow(error_authentication_exception_1.ErrorAuthenticationException);
        });
    });
    describe('removeBearerPrefix', () => {
        it('should remove Bearer prefix from token', () => {
            const result = token_service_1.default.removeBearerPrefix('Bearer my-token-here');
            expect(result).toBe('my-token-here');
        });
        it('should throw error for invalid Bearer format', () => {
            expect(() => token_service_1.default.removeBearerPrefix('InvalidToken'))
                .toThrow(error_authentication_exception_1.ErrorAuthenticationException);
        });
    });
    describe('getTokenExpiredTime', () => {
        it('should return token expired time in seconds', () => {
            const result = token_service_1.default.getTokenExpiredTime();
    ...
    describe('removeTokenFromRedis', () => {
        it('should delete token from Redis', async () => {
            ...
            const result = await token_service_1.default.removeTokenFromRedis('encrypted-token', 'pubkey456');
            // Assert
            expect(result).toBe(1);
            expect(redis_connection_1.default.del).toHaveBeenCalled();
        });
        it('should throw error when delete fails', async () => {
            // Arrange
            encrypt_decrypt_1.default.prototype.decrypt.mockReturnValue('accesskey123-pubkey456');
            redis_connection_1.default.del.mockResolvedValue(0);
            // Act & Assert
            await expect(token_service_1.default.removeTokenFromRedis('encrypted-token', 'pubkey456'))
                .rejects
                .toThrow();
        });
    });
