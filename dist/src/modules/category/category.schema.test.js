...
describe('Category Schema Validation', () => {
    describe('createCategorySchema', () => {
        it('should pass validation with valid name', () => {
            const result = category_schema_1.createCategorySchema.safeParse(category_mock_1.mockCreateCategoryData.valid);
        ...
        it('should fail validation with empty name', () => {
            const result = category_schema_1.createCategorySchema.safeParse({ name: '' });
        ...
        it('should fail validation with name less than 2 characters', () => {
            const result = category_schema_1.createCategorySchema.safeParse(category_mock_1.mockCreateCategoryData.tooShort);
            expect(result.success).toBe(false);
            if (!result.success) {
                const nameError = result.error.issues.find((issue) => issue.path.includes('name'));
                expect(nameError).toBeDefined();
        ...
        it('should fail validation with name more than 50 characters', () => {
            const result = category_schema_1.createCategorySchema.safeParse(category_mock_1.mockCreateCategoryData.tooLong);
            expect(result.success).toBe(false);
            if (!result.success) {
                const nameError = result.error.issues.find((issue) => issue.path.includes('name'));
                expect(nameError).toBeDefined();
        ...
        it('should fail validation with missing name', () => {
            const result = category_schema_1.createCategorySchema.safeParse({});
            expect(result.success).toBe(false);
        });
    });
    // ============================================
    ...
    describe('updateCategorySchema', () => {
        it('should pass validation with valid name', () => {
            const result = category_schema_1.updateCategorySchema.safeParse({ name: 'Updated Name' });
            expect(result.success).toBe(true);
            if (result.success) {
        ...
        it('should pass validation with empty object (no updates)', () => {
            const result = category_schema_1.updateCategorySchema.safeParse({});
            expect(result.success).toBe(true);
        });
        it('should fail validation with name less than 2 characters when provided', () => {
            const result = category_schema_1.updateCategorySchema.safeParse({ name: 'A' });
            expect(result.success).toBe(false);
            if (!result.success) {
                const nameError = result.error.issues.find((issue) => issue.path.includes('name'));
                expect(nameError).toBeDefined();
        ...
        it('should fail validation with name more than 50 characters when provided', () => {
            const result = category_schema_1.updateCategorySchema.safeParse({ name: 'A'.repeat(51) });
            expect(result.success).toBe(false);
            if (!result.success) {
                const nameError = result.error.issues.find((issue) => issue.path.includes('name'));
                expect(nameError).toBeDefined();
    ...
    describe('categoryIdParamSchema', () => {
        it('should pass validation with valid UUID', () => {
            const result = category_schema_1.categoryIdParamSchema.safeParse({
        ...
        it('should fail validation with invalid UUID format', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const categoryIdError = result.error.issues.find((issue) => issue.path.includes('category_id'));
    ...
    describe('categoryListQuerySchema', () => {
        it('should pass validation with empty query (uses defaults)', () => {
            const result = category_schema_1.categoryListQuerySchema.safeParse({});
            expect(result.success).toBe(true);
        });
...
//# sourceMappingURL=category.schema.test.js.map