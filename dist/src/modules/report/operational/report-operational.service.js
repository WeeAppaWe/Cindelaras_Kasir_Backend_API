...
exports.reportOperationalService = exports.getFullReport = exports.getOrderStatus = exports.getMenuPerformance = exports.getTransactionStats = exports.getShiftSummary = exports.getCashierPerformance = void 0;
...
// ============================================
const getCashierPerformance = async (req) => {
    try {
        const filter = extractFilter(req);
        const data = await report_operational_repository_1.default.getUsersWithOrderStats(filter);
        if (!data) {
            return {
        ...
        const cashiers = Array.from(userMap.values())
        ...
        console.error(`--- Report Operational Service Error: ${error.message}`);
        throw error;
    }
};
...
const getShiftSummary = async (req) => {
    try {
        const filter = extractFilter(req);
        const shifts = await report_operational_repository_1.default.getShiftsWithOrderCounts(filter);
        if (!shifts) {
            return {
                period: createPeriod(filter),
                total_shifts: 0,
...
// ============================================
const getTransactionStats = async (req) => {
    try {
        const filter = extractFilter(req);
        const orders = await report_operational_repository_1.default.getAllOrdersForPeriod(filter);
...
// ============================================
const getMenuPerformance = async (req) => {
    try {
        const filter = extractFilter(req);
        const limit = parseInt(req.query.limit) || 20;
        const menuData = await report_operational_repository_1.default.getMenuPerformanceData(filter, limit);
...
const getOrderStatus = async (req) => {
    try {
        ...
        const summary = Array.from(statusMap.entries())
            .map(([status, count]) => ({
            status,
            count,
...
const getFullReport = async (req) => {
    try {
        ...
        const [cashierData, shiftData, statsData, statusData, menuData] = await Promise.all([
            (0, exports.getCashierPerformance)(req),
            (0, exports.getShiftSummary)(req),
            (0, exports.getTransactionStats)(req),
            (0, exports.getOrderStatus)(req),
            (0, exports.getMenuPerformance)(req),
        ]);
        return {
            period: createPeriod(filter),
            cashier_summary: {
...
// EXPORT SERVICE
// ============================================
exports.reportOperationalService = {
...
exports.default = exports.reportOperationalService;
//# sourceMappingURL=report-operational.service.js.map