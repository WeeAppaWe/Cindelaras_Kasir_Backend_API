...
exports.reportInventoryService = exports.getFullReport = exports.getIngredientCard = exports.getOpnameHistory = exports.getInventoryValuation = exports.getStockAlerts = exports.getMovementSummary = exports.getCurrentStock = void 0;
...
const extractFilter = (req) => {
    return {
        ...
        ingredient_type: req.query.ingredient_type || null,
    };
};
...
// GET CURRENT STOCK
// ============================================
const getCurrentStock = async (req) => {
    try {
        const ingredientType = req.query.ingredient_type || null;
        const status = req.query.status || 'all';
        const limit = parseInt(req.query.limit) || 100;
        const ingredients = await report_inventory_repository_1.default.getAllIngredientsWithStatus(ingredientType, status, limit);
        ...
        console.error(`--- Report Inventory Service Error: ${error.message}`);
        throw error;
    }
};
...
// ============================================
const getMovementSummary = async (req) => {
    try {
        const filter = extractFilter(req);
        const movements = await report_inventory_repository_1.default.getStockMovementsForPeriod(filter);
...
// GET STOCK ALERTS
// ============================================
const getStockAlerts = async (req) => {
    try {
        // Get all ingredients
        const allIngredients = await report_inventory_repository_1.default.getAllIngredientsWithStatus(null, 'all', 500);
        ...
        const restockDates = await report_inventory_repository_1.default.getLastRestockDates(alertIds);
        const mapToAlertItem = (i) => {
            const currentStock = Number(i.stock_qty);
            const minStock = Number(i.min_stock);
...
// ============================================
const getInventoryValuation = async (req) => {
    try {
        const ingredients = await report_inventory_repository_1.default.getAllIngredientsWithStatus(null, 'all', 500);
        if (!ingredients || ingredients.length === 0) {
            return {
                total_items: 0,
                total_value: 0,
        ...
        const byIngredientType = Array.from(typeMap.entries())
            .map(([type, data]) => ({
            type,
...
// GET OPNAME HISTORY
// ============================================
const getOpnameHistory = async (req) => {
    try {
        const filter = extractFilter(req);
        const opnames = await report_inventory_repository_1.default.getStockOpnamesForPeriod(filter);
        ...
        const opnameItems = opnames.map(o => ({
...
// ============================================
const getIngredientCard = async (req) => {
    try {
        ...
        const data = await report_inventory_repository_1.default.getIngredientMovements(ingredientId, startDate, endDate);
        if (!data) {
            return null;
        }
...
// EXPORT SERVICE
// ============================================
exports.reportInventoryService = {
...
exports.default = exports.reportInventoryService;
//# sourceMappingURL=report-inventory.service.js.map