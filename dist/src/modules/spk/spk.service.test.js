...
const spk_service_1 = require("./spk.service");
const spk_repository_1 = __importDefault(require("./spk.repository"));
const spk_mock_1 = require("../../tests/mocks/spk.mock");
// Mock dependencies
...
describe('SPK Service', () => {
    ...
    beforeEach(() => {
        ...
        spk_repository_1.default.getOrderItemsWithRecipes
            .mockResolvedValue(spk_mock_1.mockOrdersWithRecipes);
        spk_repository_1.default.getAllIngredients
            .mockResolvedValue(spk_mock_1.mockAllIngredients);
        spk_repository_1.default.getLastSupplierForIngredients
            .mockResolvedValue(spk_mock_1.mockSuppliers);
    });
    // ============================================
    // RUN ANALYSIS TESTS
    // ============================================
    describe('runAnalysis', () => {
        it('should return SPK analysis response', async () => {
            const result = await (0, spk_service_1.runAnalysis)(mockRequest);
        ...
        it('should enrich items with supplier data', async () => {
            ...
            const itemWithSupplier = result.all_items.find(i => i.supplier_id !== null);
            if (itemWithSupplier) {
        ...
        it('should calculate total estimated cost', async () => {
            const result = await (0, spk_service_1.runAnalysis)(mockRequest);
            const calculatedTotal = result.all_items.reduce((sum, item) => sum + item.estimated_cost, 0);
        ...
        it('should filter by ingredient_type when specified', async () => {
            ...
            await (0, spk_service_1.runAnalysis)(rawOnlyRequest);
            expect(spk_repository_1.default.getAllIngredients).toHaveBeenCalledWith('raw');
        });
        it('should group items by supplier correctly', async () => {
            ...
            for (const group of result.by_supplier) {
                ...
                const calculatedTotal = group.items.reduce((sum, item) => sum + item.estimated_cost, 0);
        ...
        it('should apply buffer percentage correctly', async () => {
            ...
            const resultNoBuffer = await (0, spk_service_1.runAnalysis)(noBufferRequest);
            ...
            const resultHighBuffer = await (0, spk_service_1.runAnalysis)(highBufferRequest);
            ...
            if (resultNoBuffer.all_items.length > 0 && resultHighBuffer.all_items.length > 0) {
                const noBufferTotal = resultNoBuffer.all_items.reduce((sum, i) => sum + i.suggested_qty, 0);
                const highBufferTotal = resultHighBuffer.all_items.reduce((sum, i) => sum + i.suggested_qty, 0);
                expect(highBufferTotal).toBeGreaterThanOrEqual(noBufferTotal);
            }
        });
    });
});
//# sourceMappingURL=spk.service.test.js.map