...
exports.spkService = exports.runAnalysis = void 0;
const spk_repository_1 = __importDefault(require("./spk.repository"));
const wma_calculation_utility_1 = require("../../../utility/wma-calculation.utility");
// ============================================
// HELPERS
// ============================================
...
// ============================================
const extractConfig = (req) => {
    return {
        target_days: parseInt(req.query.target_days) || 7,
        buffer_percent: parseFloat(req.query.buffer_percent) || 10,
        lookback_days: parseInt(req.query.lookback_days) || 30,
    };
};
// ============================================
// TAHAP 2: FORECASTING (Recipe Explosion + WMA)
// ============================================
const calculateForecasts = async (lookbackDays, ingredientType) => {
    ...
    const orders = await spk_repository_1.default.getOrderItemsWithRecipes(startDate, endDate);
    ...
    const ingredientUsageMap = new Map(); // ingredient_id -> date -> qty
    ...
    for (const [ingredientId, dateUsageMap] of ingredientUsageMap) {
        ...
        const dailyUsages = allDates.map(date => ({
            date,
            qty: dateUsageMap.get(date) || 0,
        }));
        // Calculate WMA using utility
        const dataPoints = dailyUsages.map(d => ({ date: d.date, value: d.qty }));
        const wmaResult = (0, wma_calculation_utility_1.calculateWMAFromDataPoints)(dataPoints);
...
// TAHAP 3: CALCULATION
// ============================================
const calculateRequirements = (forecasts, ingredients, config) => {
    const calculations = [];
...
// ============================================
const groupBySupplier = (items, suppliers) => {
...
const runAnalysis = async (req) => {
    try {
        ...
        const ingredientType = req.query.ingredient_type || 'all';
        ...
        // Get all ingredients
        const ingredients = await spk_repository_1.default.getAllIngredients(ingredientType);
        ...
        const supplierData = await spk_repository_1.default.getLastSupplierForIngredients(ingredientIds);
        const supplierMap = new Map();
        if (supplierData) {
        ...
        console.error(`--- SPK Service Error: ${error.message}`);
        throw error;
    }
};
...
exports.default = exports.spkService;
//# sourceMappingURL=spk.service.js.map