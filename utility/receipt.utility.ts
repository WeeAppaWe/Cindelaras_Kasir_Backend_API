...
const getPdfLogoSource = (logoValue: string | null | undefined): string | Buffer | null => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const estimatedHeight = Math.max(
                560,
                330 + logoHeight + itemHeightEstimate + addressLines * 9 + phoneLines * 9 + headerLines * 8 + footerLines * 9
            );
            ...
            const drawDivider = (positionY: number, color = border): void => {
                doc
                    .strokeColor(color)
                    .lineWidth(0.6)
            ...
            drawAmountRow('Subtotal', formatMoney(data.total), y + 34);
            drawAmountRow('Bayar', formatMoney(data.paid_amount), y + 50);
            if (data.change_amount > 0) {
                drawAmountRow('Kembalian', formatMoney(data.change_amount), y + 66);
            }
            drawDivider(y + summaryHeight - 28, '#D1D5DB');
            drawAmountRow('TOTAL', formatMoney(data.total), y + summaryHeight - 19, true, accent);
