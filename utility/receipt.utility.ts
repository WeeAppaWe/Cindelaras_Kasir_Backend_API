...
import { formatMoney } from './format-money.utility';
import { getSupabaseStorageFolder, SUPABASE_STORAGE_BUCKET } from '../config/supabase.config';

const MAX_PDF_LOGO_BYTES = 5 * 1024 * 1024;

const getReceiptDisplayNumber = (data: ReceiptDataForGenerator): string => {
    return data.receipt || '-';
};
...
const toHtmlLines = (value: string | null | undefined): string => {
    return escapeHtml(value).replace(/\r?\n/g, '<br>');
};

const getLocalLogoPath = (logoValue: string | null | undefined): string | null => {
    ...
    const workspacePath = path.resolve(process.cwd()).toLowerCase();
    const candidatePath = absolutePath.toLowerCase();

    if (!candidatePath.startsWith(`${workspacePath}${path.sep.toLowerCase()}`)) {
        return null;
    }
...
const getDataUriLogoBuffer = (logoValue: string | null | undefined): Buffer | null => {
...
const buildSupabasePublicUrl = (objectPath: string): string | null => {
    ...
    return `${supabaseUrl}/storage/v1/object/public/${SUPABASE_STORAGE_BUCKET}/${encodedPath}`;
};

const getSupabaseLogoUrlCandidates = (logoValue: string | null | undefined): string[] => {
    const rawLogo = logoValue?.trim();
    ...
    const logoFolder = getSupabaseStorageFolder('logo');
    ...
    return Array.from(new Set(
        objectPathCandidates
            .map(buildSupabasePublicUrl)
            .filter((url): url is string => Boolean(url))
    ));
};

export const normalizeReceiptLogoValue = (logoValue: string | null | undefined): string => {
    const rawLogo = logoValue?.trim();
    if (!rawLogo) {
        return '';
    }
...
const isSupportedPdfImageBuffer = (buffer: Buffer): boolean => {
...
const getRemoteLogoBuffer = async (logoValue: string | null | undefined): Promise<Buffer | null> => {
    const rawLogo = logoValue?.trim();
    if (!rawLogo) {
        return null;
    }
...
const getPdfLogoSource = async (logoValue: string | null | undefined): Promise<string | Buffer | null> => {
 ...
 */
export const generatePdfReceipt = async (data: ReceiptDataForGenerator): Promise<string> => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const estimatedHeight = Math.max(
                620,
                390 + logoHeight + itemHeightEstimate + addressLines * 9 + phoneLines * 9 + headerLines * 8 + footerLines * 9
            );
            ...
            const drawDivider = (positionY: number, color = border, inset = 0): void => {
                doc
                    .strokeColor(color)
                    .lineWidth(0.6)
            ...
            data.items.forEach((item, index) => {
                ...
                doc
                    ...
                    .fontSize(7)
                    .fillColor(muted)
                    .text(`${item.qty} x ${formatMoney(item.price)}`, margin, y, {
                        width: contentWidth,
                    });
                y += 13;
            ...
            y += 8;
            const summaryRows = [
                { label: 'Subtotal', value: formatMoney(data.total) },
                { label: 'Bayar', value: formatMoney(data.paid_amount) },
                ...(data.change_amount > 0 ? [{ label: 'Kembalian', value: formatMoney(data.change_amount) }] : []),
            ];
            ...
            drawAmountRow('TOTAL', formatMoney(data.total), amountRowY + 13, true, accent);
 ...
 */
export const generateHtmlReceipt = (data: ReceiptDataForGenerator): string => {
    const itemsHtml = data.items.map((item) => `
        <div class="item-row">
            <div class="item-main">
                <div class="item-name">${escapeHtml(item.name)}</div>
                <div class="item-meta">${escapeHtml(item.qty)} x ${escapeHtml(formatMoney(item.price))}</div>
            </div>
            <div class="item-amount">${escapeHtml(formatMoney(item.subtotal))}</div>
        </div>
    `).join('');
    const receiptNumber = getReceiptDisplayNumber(data);
...
<body>
    <main class="receipt">
        ...
        <section class="summary-card">
            ...
            <div class="amount-row">
                <span>Bayar</span>
                <strong>${escapeHtml(formatMoney(data.paid_amount))}</strong>
            </div>
            ${data.change_amount > 0 ? `
            <div class="amount-row">
                <span>Kembalian</span>
                <strong>${escapeHtml(formatMoney(data.change_amount))}</strong>
            </div>` : ''}
