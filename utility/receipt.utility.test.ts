import { generateHtmlReceipt } from './receipt.utility';
...
describe('Receipt Utility', () => {
    ...
    const sampleReceiptData: ReceiptDataForGenerator = {
    ...
    describe('generateHtmlReceipt', () => {
        it('should generate HTML with store name', () => {
            const result = generateHtmlReceipt(sampleReceiptData);
        ...
        it('should not use order ID as receipt number fallback', () => {
            const result = generateHtmlReceipt({
                ...sampleReceiptData,
                receipt: null,
            });
        ...
        it('should NOT show change for zero change amount', () => {
            const dataNoChange: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                change_amount: 0,
            };
            const result = generateHtmlReceipt(dataNoChange);
            expect(result).not.toContain('Kembalian');
        });

        it('should NOT show customer name when not provided', () => {
            const dataNoCustomer: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                customer_name: undefined,
            };
            const result = generateHtmlReceipt(dataNoCustomer);
        ...
        it('should handle QRIS payment type', () => {
            const dataQris: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                payment_type: 'QRIS',
                paid_amount: 35000,
                change_amount: 0,
            };
            const result = generateHtmlReceipt(dataQris);
            expect(result).toContain('QRIS');
        });

        it('should handle empty items array', () => {
            const dataNoItems: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                items: [],
                total: 0,
                paid_amount: 0,
            ...
            const result = generateHtmlReceipt(dataNoItems);
            expect(result).toContain('<!DOCTYPE html>');
            // Should not throw error
        });

        it('should handle special characters in store name', () => {
            const dataSpecialChars: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                store_name: 'Toko & Warung <Test>',
            };
            const result = generateHtmlReceipt(dataSpecialChars);
    ...
    describe('Edge Cases', () => {
        it('should handle very long store name', () => {
            const dataLongName: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                store_name: 'A'.repeat(100),
            };
            const result = generateHtmlReceipt(dataLongName);
            expect(result).toContain('A'.repeat(100));
        });

        it('should handle very long item name', () => {
            const dataLongItem: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                items: [
                    { name: 'Menu dengan nama yang sangat panjang sekali', qty: 1, price: 10000, subtotal: 10000 },
                ],
            };
            const result = generateHtmlReceipt(dataLongItem);
            expect(result).toContain('Menu dengan nama yang sangat panjang sekali');
        });

        it('should handle large quantities', () => {
            const dataLargeQty: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                items: [
                    { name: 'Item Test', qty: 9999, price: 1000, subtotal: 9999000 },
                ],
            };
            const result = generateHtmlReceipt(dataLargeQty);
            expect(result).toContain('9999');
        });

        it('should handle large amounts', () => {
            const dataLargeAmount: ReceiptDataForGenerator = {
                ...sampleReceiptData,
                total: 999999999,
                paid_amount: 1000000000,
                change_amount: 1,
            };
            const result = generateHtmlReceipt(dataLargeAmount);
            expect(result).toContain('Rp');
        });
    });
});
